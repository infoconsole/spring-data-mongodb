...
public class BsonUtils {
	...
	public static Object toJavaType(BsonValue value) {
		...
		switch (value.getBsonType()) {
			...
			case DB_POINTER:
				throw new IllegalArgumentException("cannot convert type db_pointer to java type.");
			case BINARY:
				return value.asBinary().getData();
			case DATE_TIME:
				return new Date(value.asDateTime().getValue());
			case SYMBOL:
				throw new IllegalArgumentException("cannot convert type symbol to java type.");
			case ARRAY:
